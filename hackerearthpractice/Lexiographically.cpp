...
void unionSet(ll i, ll j) {    // merging sets containing i and j
    if(!isSameSet(i,j)) {
        ...
        if(rank_element[x] > rank_element[y]){
            ...
            lex[x] = min(lex[x],lex[y]);
        }
        else {
            parent[x] = y;
            sz_set[y] += sz_set[x];  //increasing the size
            lex[y] = min(lex[x],lex[y]);
...
int main() 
{
    ...
    ll n ;
    n = 26;   // representing the 26 alphabets
    ...
    fo(i,0,a.size()) {
        unionSet((ll)(a[i]-'a'),(ll)(b[i]-'a'));
    }

    fo(i,0,c.size()){
        //ll idx = findSet[c[i]-'a'];
        c[i] = (char)(lex[findSet((ll)(c[i]-'a'))] + 'a');
    }
    cnl(c);
    return 0;
}