...
void process(ll vtx, vector<vector<pair<ll,ll>>>& AdjList) { // so, we use -ve sign to reverse the sort order
    taken[vtx] = 1;
...
int main() 
{
    ...
    while(t--)
    {
        ...
        mst_cost += w, process(u,adjList); // take u, process all edges incident to u
        }
