...
ll sizeeee(vector<char>& a,vector<char>& b){

    unordered_set<char,custom_hash> settt;

    ll ans  = +(a.size() + b.size());

    for(char ch: a) settt.insert(ch);

    for(char ch: b) settt.insert(ch);
...
/////////////////////
ll calc(unordered_map<string,vector<char>>& prolly) {

    ll me = 0;
    for(auto& str1 : prolly){
        ll s1 = str1.second.size();
        for(auto& str2: prolly){
            ll s2 = str2.second.size();
            ll s3 = sizeeee(str1.second,str2.second);
            me += (s1-s3)*(s2-s3);
        }
    }
    return me;
...
void solve() {
    ...
    unordered_map<string,vector<char>> prolly;
    ...
    for(int i = 0 ; i < n ; i++) {
        ...
        string randd = yo.substr(1);

        prolly[randd].push_back(yo[0]);
    }

    cout<<calc(prolly)<<endl;
}
