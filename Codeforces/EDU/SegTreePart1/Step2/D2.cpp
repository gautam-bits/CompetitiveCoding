...
struct SegTree
{
    ...
    ll find(ll node,ll L,ll R,ll x) {
        ...
        //csp(lsum);cnl(k+1);
        if(lmax < x) {
            return find(node*2+1,M+1,R,x);
        }

        else return find(node*2,L,M,x);
    }

    vi find2(ll node,ll L,ll R,ll i ) {
        if(L == i) return {node,L,R};
    }
    ...
    cin.tie(NULL);
        ...
        fo(i,0,m) {
            ...
            if(a == 1) {
                ...
                if(myseg.query(1,n).max < b) cnl(-1);
                //fo(j,1,4*n) csp(myseg.st[j].sum);
                //cnl("");
                else cnl(myseg.find(1,1,n,b)-1);
                //cnl("yo");
            }
        }
